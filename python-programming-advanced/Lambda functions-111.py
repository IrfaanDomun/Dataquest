...
def isPalindrome(str):
    return str == str[::-1]
# Your code goes here
phrase_palindrome = isPalindrome(able_string)
...
def easy_patterns(str):
    return len( [ i for i in passwords if str in i])
...
# Your code goes here
palindrome_passwords = [ i for i in passwords if is_palindrome(i)]
...
medium_passwords  = list(filter(lambda x : 5<len(x)<11, passwords))
strong_passwords  = list(filter(lambda x : 10<len(x)  , passwords))
