...
class Chopstick(object):
    def __init__(self, length):
        ...
        self.trials = [Trial(i) for i in chopsticks if int(i[2]) == self.length]
...
class Chopstick(object):
    ...
    def avg_efficiency(self):
        return sum([ i.efficiency for i in self.trials])/len(self.trials)
    ...
    def __init__(self, length):
        ...
        for row in chopsticks:
            if int(row[2]) == self.length:
                ...
                if -1 not in [trial.efficiency,trial.individual,trial.chopstick_length]:
...
chopstick_list = list(map(Chopstick,chopstick_lengths))
